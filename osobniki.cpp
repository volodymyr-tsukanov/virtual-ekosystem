...
Glon::Glon():Organizm(GeneratorLosowy::losujPomiedzy(UST.glonZycieMin, UST.glonZycieMax), UST.glonLimitPosilkow, UST.glonKosztPotomka), Mieszkaniec(UST.znakGlon)
{

}

Grzyb::Grzyb():Organizm(GeneratorLosowy::losujPomiedzy(UST.grzybZycieMin, UST.grzybZycieMax), UST.grzybLimitPosilkow, UST.grzybKosztPotomka), Mieszkaniec(UST.znakGrzyb)
{

}

Bakteria::Bakteria():Organizm(GeneratorLosowy::losujPomiedzy(UST.bakteriaZycieMin, UST.bakteriaZycieMax), UST.bakteriaLimitPosilkow, UST.bakteriaKosztPotomka), Mieszkaniec(UST.znakBakteria)
{
...
ZamiarMieszkanca Glon::wybierzAkcje(Sasiedztwo sasiedztwo){
    ...
    if(zywy() && paczkujacy() && sasiedztwo.ile(PUSTKA) > 0) return ZamiarMieszkanca(POTOMEK, sasiedztwo.losujSasiada(PUSTKA));
...
ZamiarMieszkanca Grzyb::wybierzAkcje(Sasiedztwo sasiedztwo){
    ...
    if(zywy() && glodny() && sasiedztwo.ile(TRUP) > 0) return ZamiarMieszkanca(ROZKLAD, sasiedztwo.losujSasiada(TRUP));
...
ZamiarMieszkanca Bakteria::wybierzAkcje(Sasiedztwo sasiedztwo){
    ...
    if(zywy() && glodny() && sasiedztwo.ile(GLON) > 0) return ZamiarMieszkanca(POLOWANIE, sasiedztwo.losujSasiada(GLON));
    if(zywy() && glodny() && sasiedztwo.ile(BAKTERIA) > 0) return ZamiarMieszkanca(POLOWANIE, sasiedztwo.losujSasiada(BAKTERIA));
