...
{
    if(polozenie != NIGDZIE) * elementWewnetrzny(polozenie) = rodzaj;
}

RodzajMieszkanca Sasiedztwo::ktoJestSasiadem(Polozenie polozenie)
{
    if(polozenie != NIGDZIE) return * elementWewnetrzny(polozenie);
...
void Sasiedztwo::zmienindeksWgPolozenia(Polozenie polozenie, long &wiersz, long &kolumna)
{
...
void Sasiedztwo::zmienindeksWgPolozenia(Polozenie polozenie, unsigned int &wiersz, unsigned int &kolumna)
{
    long w = static_cast<long>(wiersz);
    long k = static_cast<long>(kolumna);

    zmienindeksWgPolozenia(polozenie, w, k);
